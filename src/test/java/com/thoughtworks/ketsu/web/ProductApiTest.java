...
public class ProductApiTest extends ApiSupport {
    ...
    public void should_get_one_product_successfully() {
        ...
        verifyResponseProdInfo(product, getOneUrl, prodInfo);
    }

    private void verifyResponseProdInfo(Product product, String prodUrl, Map prodInfo) {
        assertThat(prodInfo.get("uri").toString(), is(prodUrl));
    ...
    public void should_get_all_successfully() {
        ...
        verifyResponseProdInfo(product, productsBaseUrl + "/" + product.getId(), prodInfo);
    }

    @Test
    public void should_return_nothing_when_get_all_given_empty_db() {
        Response response = target(productsBaseUrl).request().get();
